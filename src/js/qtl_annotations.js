...
export const QtlAnnotations = function (userConfig) {
  var defaultConfig = {
    border: false,
  ...
  var setupSNPAnnotations = function (
    ...
    snpsAnnotations.on("contextmenu", function (d) {
      ...
      //     return d[0] + ": " + d[1];
      //   });
      // const tooltip = document.getElementById("clusterPopover");
      //Apply the boostrap popover function
      // TODO: Fix this
      // let $clusterPopover = $("#clusterPopover");
  ...
  var setupQTLAnnotations = function (
    ...
    qtlAnnotations.select("rect.qtl-selector").style("fill", function (d) {
      return d.color;
    });

    let debugQtlLableBoxes = false;

    if (debugQtlLableBoxes) {
      //Rectanges to check size of labels
      qtlAnnotations
        .select("rect.test")
        .attr("x", (d) => (d.displayLabel ? xLabel(d) : 0))
        .attr("y", (d) =>
          d.displayLabel
            ? y(d.midpoint) -
    ...
    qtlCountParentGroup.each(function (d) {
      if (_.has(newNodes, d.index)) {
        if (_.has(oldNodes, d.parentIndex)) {
          let oldNode = oldNodes[d.parentIndex];
    ...
    qtlAnnotations
      ...
      .style(
        "visibility",
        circleFontSize < 2 * bandWidth ? "visible" : "hidden"
      )
      .text(function (d) {
        return d.count;
      });
    ...
    qtlLabelAnnotations
      ...
      .attr("transform", function (d) {
        return (
          "translate(" + (xLabel(d) + 0.5 * bandWidth) + "," + textYPos(d) + ")"
        );
      });
